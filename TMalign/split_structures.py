...
def readScores(fl):
    ...
    for f in fl:
        ...
        for line in lines:
            ...
            regexp_chain_1 = re.compile ("^(REMARK)[\s]+(Chain\s1:)([A-Za-z0-9_]+)") # In our case this is the model that have been generated
            regexp_chain_2 = re.compile ("^(REMARK)[\s]+(Chain\s2:)([A-Za-z0-9_]+)") # In our case this is the template
...
def tSmain():
    ...
    scores_fout = open("Scores.txt", "w")
    for obj in scoreList:
        score = "%s\t%s\t%s\t%s\n" %(obj.c1, obj.c2, obj.RMSD, obj.TMscore)
        scores_fout.write(score)
    cleanAll()