...
def plot_ROC_all(results, roc_name):
    fig = plt.figure()
    line_1 = plt.plot(results['Random_Forest']['fpr'], results['Random_Forest']['tpr'], label='RF', linewidth=1.2)
    line_2 = plt.plot(results['Decision_Tree']['fpr'], results['Decision_Tree']['tpr'], label='DT', linewidth=1.2)
    line_3 = plt.plot(results['Linear_SVM']['fpr'], results['Linear_SVM']['tpr'], label='SVM linear', linewidth=1.2)
    line_4 = plt.plot(results['RBF_SVM']['fpr'], results['RBF_SVM']['tpr'], label='SVM RBF', linewidth=1.2)
    line_5 = plt.plot(results['Nearest_Neighbors']['fpr'], results['Nearest_Neighbors']['tpr'], label='k-NN', linewidth=1.2)
    line_6 = plt.plot(results['AdaBoost']['fpr'], results['AdaBoost']['tpr'], label='AdaBoost', linewidth=1.2)
    line_7 = plt.plot(results['Naive_Bayes']['fpr'], results['Naive_Bayes']['tpr'], label='NB', linewidth=1.2)
    line_8 = plt.plot(results['LDA']['fpr'], results['LDA']['tpr'], label='LDA', linewidth=1.2)
