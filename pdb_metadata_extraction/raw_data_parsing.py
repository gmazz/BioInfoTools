    ...
    for rec in records:
        ...
        if vals:
            ...
            if len(raw_data) == 5:
                ...
                rec_dir[id] = {'chain': raw_data[0], 'host': raw_data[1], 'location': raw_data[2], 'year': year, 'subtype': subtype, 'line_length': 5}
            elif len(raw_data) == 4:
                year = clear_year(raw_data[3])
                rec_dir[id] = {'chain': raw_data[0], 'host': '', 'location': raw_data[1], 'year': year, 'subtype': subtype, 'line_length': 4}
            elif len(raw_data) == 3:
                year = clear_year(raw_data[2])
                rec_dir[id] = {'chain': raw_data[0], host: '', 'location': raw_data[1], 'year': year, 'subtype': subtype, 'line_length': 3}
            else:
...
#    if v:
#        print "%s,%s,%s,%s,%s,%s" %(k, v['chain'], v['subtype'], v['host'], v['location'], v['year'])