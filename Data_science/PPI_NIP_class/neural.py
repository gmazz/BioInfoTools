...
class NeuralNetClassifier():
    ...
    def __init__(self, lmda=1e-5, hidden_layer_length_list=[], epochs=2, learning_rate=0.5, learning_acceleration=1.05,
                 learning_backup=0.5, momentum_rate=0.1):
        self.nn = py_nn()
        self.nn.lmda = lmda
        if hidden_layer_length_list:
            self.nn.hidden_layer_length_list = hidden_layer_length_list
        self.nn.epochs = epochs
        self.nn.learning_rate = learning_rate
        self.nn.learning_acceleration = learning_acceleration
        self.nn.learning_backup = learning_backup
        self.nn.momentum_rate = momentum_rate

    def get_params(self, deep=True):
        return {"lmda": self.nn.lmda, "hidden_layer_length_list": self.nn.hidden_layer_length_list, "epochs": self.nn.epochs,
                "learning_rate": self.nn.learning_rate, "learning_acceleration": self.nn.learning_acceleration,
                "learning_backup": self.nn.learning_backup, "momentum_rate": self.nn.momentum_rate}

    def set_params(self, **parameters):
        for param, val in parameters.items():
            self.nn.setattr(param, val)
    ...
    def fit(self, X, y):
        try:
            ...
            self.nn.fit(X, np.array(map(lambda x: [x==i for i in xrange(len(np.unique(y)))], y)))

    def predict(self, X):
        try:
            len(X[0])
            res = self.nn.nn_predict(X)[1]
        except TypeError:
            res = self.nn.nn_predict(np.array([X]))[1]
        return np.array(map(lambda x: (x==max(x)).nonzero()[0][0], res))

    def score(self,X,y):
        return metrics.accuracy_score(y,self.predict(X))

class py_nn():  # Author: Nathan Wenzel, Edge Solutions, Inc. http://www.edgesolutions.com/
    """
