...
def correlation(l1, l2, id_tuple):
    if len(l1) == len(l2):
        l1 = np.array(l1)
        l2 = np.array(l2)
        data = np.array([l1, l2])
...
def main():
    ...
    corr_file = open('corr_coeff.txt', 'w+')
    euc_file = open('eucl_dist.txt', 'w+')
    ...
    for i in pdb_pairs:
        ...
        corr_coeff = correlation(l1, l2, id_tuple)

        euc_file.write("%s\t%s\t%s\n" % (id_tuple[0], id_tuple[1], euc_dist))
        corr_file.write("%s\t%s\t%s\n" % (id_tuple[0], id_tuple[1], corr_coeff))
